...
class FollowingDialog extends StatefulWidget {
  ...
  FollowingDialog({Key key, this.title, this.myProfile}) : super(key: key);

  FollowingState createState() => new FollowingState(myProfile: myProfile);
}

class FollowingState extends State<FollowingDialog> {
  Follows follows, follows2;
  List<Register> users = [];
  Profile myProfile;

  FollowingState({this.myProfile});
  ...
  Widget build(BuildContext context) {
    for (Profile p in myProfile.follow.followings)
      print("profile: ${p.user.username}");
    return new Scaffold(
      appBar: new AppBar(
          backgroundColor: Colors.black54,
          centerTitle: true,
          title: new Following(profile: myProfile)),
      body: new GridView.count(
        padding: const EdgeInsets.all(20.0),
        crossAxisCount: 3,
        children:
            new List.generate(myProfile.follow.followings.length, (index) {
          print(
              '${myProfile.follow.followings[index].user.username}\n${index}');
          return new Padding(
              padding: const EdgeInsets.all(12.0),
              child: Bubbles(
                myProfile: myProfile,
                userinformation: myProfile.follow.followings[index],
                followings: myProfile.follow.followings[index].follow.followers,
              ));
        }),
      ),
    );
...
class Bubbles extends StatelessWidget {
  ...
  Bubbles(
      {this.userinformation, this.myProfile, this.follows, this.followings: 0});
