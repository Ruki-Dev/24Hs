...
class MutualDialog extends StatefulWidget {
  final String title;
  final Profile myProfile;

  MutualDialog({Key key, this.title, this.myProfile}) : super(key: key);

  MutualState createState() => new MutualState(myProfile: myProfile);
}

class MutualState extends State<MutualDialog> {
  Follows follows, follows2;
  List<Register> users = [];
  Profile myProfile;

  MutualState({this.myProfile});
    ...
    return new Scaffold(
      ...
      body: new GridView.count(
        ...
        children: new List.generate(
            myProfile.friends.getMutualFriend(myProfile).length, (index) {
          return new Padding(
              padding: const EdgeInsets.all(12.0),
              child: Bubbles(
                myProfile: myProfile,
                userinformation:
                    myProfile.friends.getMutualFriend(myProfile)[index],
                followers: myProfile.friends
                    .getMutualFriend(myProfile)[index]
                    .follow
                    .followers,
              ));
        }),
