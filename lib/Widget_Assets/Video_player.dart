...
import 'package:twenty_four_hours/Widget_Assets/video_player_full.dart';
...
class Video_Player extends StatefulWidget {
  ...
  Video_Player({this.profile, this.vid, this.myProfile});

  VideoPlayerState createState() =>
      VideoPlayerState(profile: profile, myProfile: myProfile, vid: vid);
}

class VideoPlayerState extends State<Video_Player>
    with TickerProviderStateMixin {
  final Profile profile;
  final Profile myProfile;
  final Videos vid;

  VideoPlayerState({this.profile, this.vid, this.myProfile});
                  ...
                  flexibleSpace: new FlexibleSpaceBar(
                    ...
                    DecoratedBox(
                      decoration: BoxDecoration(
                          color:
                              Color.alphaBlend(Colors.black87, Colors.black54)),
                    ),
                    MyVidIntro(),
                  ])),
                ),
  ...
  Future FullScreen(BuildContext context, Videos v) async {
    //Navigator.pop(context);
    Navigator.of(context).push(new MaterialPageRoute<Null>(
        builder: (BuildContext context) {
          return new Video_Player_full(
            cont: _cont,
            myProfile: myProfile,
            vid: v,
            profile: profile,
...
lue.aspectRatio, child: VideoPlayer(controller))
  ...
  void initState() {
    ...
    _cont = VideoPlayerController.network(vid.streamlink)
